...
function App() {
    ...
    const selectedPrompt = useMemo(() => prompts.find((p) => p.selected)!, [prompts]);
    ...
    const sendMessage = async (text: string) => {
        ...
        try {
            ...
            const prompt = `${prePrompt}\n${t(selectedPrompt.text)}${conversationHistory}\n-${text}`;
            ...
            if (user?.settings?.voiceAnswer) {
                ...
                speak({
                    ...
                    voice: voiceOptions.find((voice) => voice.name === user?.settings?.voiceChoice),
                });
            }
        } catch (error: any) {
            console.log(error);
    ...
    return (
        <div className="App">
            ...
            />
            <Grid pl={0.2} pr={0.2} display="flex" flexDirection="column" flex={1} height="100dvh">
            ...
            <AlertDialog open={logoutAlert} setOpen={setLogoutAlert} title={t('logoutTitle')} onConfirm={signOut} />
            <SettingsDialog
                user={user}
                open={settingsOpen}
